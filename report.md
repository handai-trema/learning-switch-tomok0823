...
ここでは, startハンドラ内で定義された ```@fdbs = {}``` に対して, switch_readyハンドラ内でFDBのインスタンス変数を作成している.一つのスイッチ内でのホスト間の送受信について扱う場合と比較して,複数のスイッチを扱う今回は ```datapath_id``` をキーとしたハッシュを用いてインスタンス変数を作成することで,新しいスイッチが接続された際に, 新たにFDBを作成することができる.  
